...
def data_augmentation_imgaug3():  # Pieta
    ...
    seq = iaa.Sequential([
        ...
        iaa.ContrastNormalization((0.8, 1.6)),  # sharpen images)
...
for trial in range(seedStart, seedEnd, 1):
    ...
    list_trials_acc.append(acc)
    list_trials_val_acc.append(val_acc)
    list_trials_loss.append(loss)
    list_trials_val_loss.append(val_loss)


np.save('list_trials_acc', np.array(list_trials_acc))
np.save('list_trials_val_acc',np.array(list_trials_val_acc))
np.save('list_trials_loss',np.array(list_trials_loss))
