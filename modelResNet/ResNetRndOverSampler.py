...
def plot_metrics(history, i):
    ...
    for n, metric in enumerate(metrics):
        ...
        plt.legend()
        plt.savefig(os.path.join('performance_vizualizations', 'MetricsRND' + str(i)))
...
class PerformanceVisualizationCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        fig.savefig(os.path.join(self.image_dir, f'confusion_matrix_epoch_{epoch}RND'))

        # plot and save roc curve
        fig, ax = plt.subplots(figsize=(16, 12))
        plot_roc(y_true, y_pred, ax=ax)
        # fig.savefig(os.path.join(self.image_dir, f'roc_curve_epoch_{epoch}'))
        fig.savefig(os.path.join(self.image_dir, f'roc_curve_epoch_{epoch}RND'))
        plt.close()
...
class PerformanceVisualizationCallbackTest(Callback):
    ...
    def on_test_end(self, epoch, logs={}):
        ...
        plot_confusion_matrix(y_true, y_pred_class, ax=ax)
        # fig.savefig(os.path.join(self.image_dir, f'confusion_matrix_epoch_{epoch}'))
        fig.savefig(os.path.join(self.image_dir, 'confusion_matrix_epoch_testRND'))

        # plot and save roc curve
        fig, ax = plt.subplots(figsize=(16, 12))
        plot_roc(y_true, y_pred, ax=ax)
        # fig.savefig(os.path.join(self.image_dir, f'roc_curve_epoch_{epoch}'))
        fig.savefig(os.path.join(self.image_dir, 'roc_curve_epoch_testRND'))
        plt.close()
...
plt.savefig(os.path.join('DataSamplesDistributionPriorRND'))
plt.close()
...
plt.savefig(os.path.join('DataSamplesDistributionRND'))
plt.close()
...
for im, lab in zip(imgs_train, train_labels_enc):
    ...
    if (lab == 4):
        ...
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        name = str(n) + '.png'
        cv2.imwrite(os.path.join('rnd', name), img)
...
for i in range(seedStart, seedEnd, 1):
    ...
    classfic_report.savefig(os.path.join(image_dir, 'test_plot_classif_reportRND.png'))
