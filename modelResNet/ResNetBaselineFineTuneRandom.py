...
class PerformanceVisualizationCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        fig.savefig(os.path.join(self.image_dir, f'confusion_matrix_epoch_{epoch}Baseline'))

        # plot and save roc curve
        fig, ax = plt.subplots(figsize=(16, 12))
        plot_roc(y_true, y_pred, ax=ax)
        # fig.savefig(os.path.join(self.image_dir, f'roc_curve_epoch_{epoch}'))
        fig.savefig(os.path.join(self.image_dir, f'roc_curve_epoch_{epoch}Baseline'))
        plt.figure().clear()
        plt.close()
        plt.cla()
        plt.clf()
...
for trial in range(seedStart, seedEnd, 1):
    ...
    train_ds = imgs_train
    print("CONCATENATEDDDDDDDDDD", train_ds[0])
    ...
    model.compile(loss='sparse_categorical_crossentropy',
                  ...
                  # optimizer=optimizers.SGD(learning_rate=initial_LR, decay=1e-6, momentum=0.9, nesterov=True),
                  metrics=['accuracy'])
    ...
    np.save('baseline_list_trials_val_acc_np', list_trials_val_acc_np)
    np.save('baseline_list_trials_val_loss_np', list_trials_val_loss_np)
    np.save('baseline_list_trials_val_acc', np.array(list_trials_val_acc))
    np.save('baseline_list_trials_val_loss', np.array(list_trials_val_loss))
    np.save('baseline_list_trials_val_acc_nonnp', list_trials_val_acc)
    np.save('baseline_list_trials_val_loss_nonnp', list_trials_val_loss)
    ...
    results = model.evaluate(x=imgs_test, y=test_labels_enc, batch_size=146, callbacks=[performance_cbkt])
    ...
    classfic_report.savefig(os.path.join(image_dir, 'test_plot_classif_report_baseline' + str(trial) + '.png'))
    ...
    model.summary()
    model.save('modelResNetBaseline' + str(trial))
