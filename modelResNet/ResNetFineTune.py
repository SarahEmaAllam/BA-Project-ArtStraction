...
def plot_metrics(history, i):
    ...
    metrics = ['precision0', 'recall0', 'precision1', 'recall1', 'precision2', 'recall2', 'precision3', 'recall3', 'precision4', 'recall4', 'precision5', 'recall5']
    for n, metric in enumerate(metrics):
        ...
        plt.legend()
        plt.savefig(os.path.join('performance_vizualizations', 'Metrics'+str(i)))
...
class PerformanceVisualizationCallbackTest(Callback):
    ...
    def on_test_end(self, epoch, logs={}):
        ...
        plot_confusion_matrix(y_true, y_pred_class, ax=ax)
        #fig.savefig(os.path.join(self.image_dir, f'confusion_matrix_epoch_{epoch}'))
        fig.savefig(os.path.join(self.image_dir, 'confusion_matrix_epoch_test'))

        # plot and save roc curve
        fig, ax = plt.subplots(figsize=(16, 12))
        plot_roc(y_true, y_pred, ax=ax)
        #fig.savefig(os.path.join(self.image_dir, f'roc_curve_epoch_{epoch}'))
        fig.savefig(os.path.join(self.image_dir, 'roc_curve_epoch_test'))
        plt.close()
        
       

class PerformanceVisualizationCallbackTestF1(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}): 
        print('======================================inside F1 SCore====================================')   
        # plot and save F1 score
        print("Y PRED", np.round(np.argmax(y_pred, axis=1)))
        print("Y TRUE", np.round(y_true))
        updated_y = f1_score(np.round(y_true), np.round(np.argmax(y_pred, axis=1)), average='weighted')
        print ("F1 score", updated_y)
        plt.ion()
        figure, ax = plt.subplots(figsize=(16, 12))
        line1, = ax.plot(epoch, updated_y)
        plt.title("F1 score across epochs",fontsize=25)
        plt.xlabel("Epoch",fontsize=18)
        plt.ylabel("F1 score",fontsize=18)
        
        
        line1.set_xdata(epoch)
        line1.set_ydata(updated_y)
        figure.canvas.draw()
        figure.canvas.flush_events()
        #fig.savefig(os.path.join(self.image_dir, f'roc_curve_epoch_{epoch}'))
        fig.savefig(os.path.join(self.image_dir, 'F1_score_epoch_test'))
...
for i in range(seedStart, seedEnd, 1):
    ...
    performance_cbktf1 = PerformanceVisualizationCallbackTestF1(
                      model=model,
                      validation_data=test_data,
                      image_dir='performance_vizualizations')
...
# plot and save F1 score
 ...
 # plot and save classification report
    ...
    classfic_report.savefig(os.path.join(image_dir, 'test_plot_classif_report.png'))
