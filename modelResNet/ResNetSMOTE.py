...
plt.savefig(os.path.join('DataSamplesDistributionPrior'))
plt.close()
...
plt.savefig(os.path.join('DataSamplesDistributionSMOTEd'))
plt.close()
...
for im, lab in zip(imgs_train, train_labels_enc):
    ...
    if (lab == 4):
        ...
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        name = str(n) + '.png'
        cv2.imwrite(os.path.join('smoted', name), img)
...
for i in range(seedStart, seedEnd, 1):
    ...
    model.compile(loss='sparse_categorical_crossentropy',
                  ...
                  optimizer=optimizers.SGD(learning_rate=0.000255, decay=1e-6, momentum=0.9, nesterov=True),
                  metrics=['accuracy'])
    ...
    history = model.fit(x=imgs_train, y=train_labels_enc,
                        ...
                        callbacks=[es, callback_scheduler_cycle2, performance_cbk, CustomMomentumScheduler(scheduler_momentum)],
                        verbose=1)
